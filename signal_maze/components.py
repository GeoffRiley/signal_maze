...
class Wire(Component):
    ...
    def process_signal(self, signal: Signal) -> List[Signal]:
        """Allow signal to continue in the same direction."""
        return [signal]
...
class Switch(Component):
    ...
    def __init__(self, x: int, y: int, state: bool = True):
        """Initialize a switch at position (x, y) with the initial state."""
    ...
    def toggle(self) -> None:
        """Toggle the switch between open and closed."""
        self.state = not self.state
        self.symbol = '○' if self.state else '●'

    def process_signal(self, signal: Signal) -> List[Signal]:
        """Allow signal through if open, block if closed."""
        return [signal] if self.state else []


class Blocker(Component):
    """Fixed the blocker that always stops signals."""
...
class Splitter(Component):
    ...
    def process_signal(self, signal: Signal) -> List[Signal]:
        """Split the signal into multiple directions."""
        signals = []
        ...
        if self.split_type == '+':
            ...
            # Split perpendicular to an incoming direction
