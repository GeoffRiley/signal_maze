...
from .components import Source, Exit
...
class Game:
    ...
    def _scan_special_components(self) -> None:
        """Scan the grid for sources and exits."""
        self.sources = []
        self.exits = []
    ...
    def simulate_signals(self) -> List[Signal]:
        ...
        while active_signals and steps < self.max_signal_steps:
            ...
            for signal in active_signals:
                ...
                # Check if the signal leaves the grid
                if not self.grid.is_valid_position(next_x, next_y):
                    continue  # Signal exits grid

                signal.move()

                if component := self.grid.get_component(signal.x, signal.y):
    ...
    def display(self) -> str:
        ...
        status_lines = [
            ...
            f"Exits reached: {sum(bool(e.reached)
                              for e in self.exits)}/{len(self.exits)}",
        ]
